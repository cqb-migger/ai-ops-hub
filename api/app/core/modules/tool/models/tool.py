...
class Tool(Base):
    ...
    mcp_type = Column(String(50), nullable=True)
    mcp_stdio_command = Column(Text, nullable=True)
    mcp_stdio_args = Column(ARRAY(Text), nullable=True, server_default='{}')
    mcp_stdio_env = Column(JSONB, nullable=True)
    mcp_stdio_env_passthrough = Column(ARRAY(Text), nullable=True, server_default='{}')
    mcp_stdio_work_dir = Column(Text, nullable=True)
    mcp_http_url = Column(Text, nullable=True)
    mcp_http_bearer_token_env = Column(Text, nullable=True)
    mcp_http_headers = Column(JSONB, nullable=True)
    mcp_http_headers_from_env = Column(JSONB, nullable=True)
