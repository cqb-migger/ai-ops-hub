...
async def create_user(user_in: UserCreate, db: AsyncSession = Depends(get_db)):
    """Create a new user."""
    # Service layer now handles the check for existing email and raises HTTPException
...
async def update_user(user_id: uuid.UUID, user_in: UserUpdate, db: AsyncSession = Depends(get_db)):
    ...
    # Service layer now handles potential data conflicts and raises HTTPException
